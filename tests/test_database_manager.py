            ...
            for i in range(5):
                record = UserRecord(
                    ...
                    score=ExperimentScore(
                        total=80 + i * 2, scientific=0, procedural=0, safety=0
                    ),
                    step_records=[],
                    context={},
                    curve_data={},
...
def benchmark_performance():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # 性能验证
            assert count / write_time > 100, (
                f"写入性能不足: {count / write_time:.0f} records/s"
            )
            assert len(loaded_records) / read_time > 1000, (
                f"读取性能不足: {len(loaded_records) / read_time:.0f} records/s"
            )
