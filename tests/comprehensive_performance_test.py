...
class PerformanceTestSuite:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                for i in range(1000):
                    record = UserRecord(
                        ...
                        score=ExperimentScore(
                            total=85, scientific=0, procedural=0, safety=0
                        ),
                        step_records=[],
                        context={},
                        curve_data={},
                ...
                self.results["database"] = {
                    "status": "PASS"
                    if write_rate > 100 and read_rate > 1000
                    else "FAIL",
    ...
    def generate_report(self):
        ...
        if "numba" in self.results:
            ...
            print(
                f"{'Numba加速':<20} {r['status']:<10} 平均加速 {r['average_speedup']}"
            )
        ...
        if "cache" in self.results:
            ...
            print(
                f"{'缓存系统':<20} {r['status']:<10} {r['write_ops_per_sec'] / 1000:.1f}K写/s, {r['read_ops_per_sec'] / 1000:.1f}K读/s"
            )
        ...
        if "database" in self.results:
            ...
            print(
                f"{'数据库':<20} {r['status']:<10} {r['write_rate']:.0f}写/s, {r['read_rate']:.0f}读/s"
            )
        ...
        if "memory" in self.results:
            ...
            print(
                f"{'内存占用':<20} {r['status']:<10} {r['usage_mb']:.1f}MB (目标<{r['target_mb']}MB)"
            )
