...
class UnifiedLogger:
    ...
    def log_performance(self, operation: str, component: str, duration: float, **kwargs) -> None:
        ...
        self._format_message(f"性能: {operation} 耗时 {duration:.3f}秒", level, **kwargs)
        getattr(self.logger, level.value.lower())(self._format_message(f"性能: {operation} 耗时 {duration:.3f}秒", level, **kwargs))
...
def log_performance(operation: str, component: str, **kwargs):
    """性能日志装饰器"""
    def decorator(func):
        def wrapper(*args, **func_kwargs):
            logger = get_unified_logger()
            ...
            try:
                result = func(*args, **func_kwargs)
                duration = (datetime.now() - start_time).total_seconds()
                logger.log_performance(operation, component, duration, **kwargs)
                return result
            except Exception as e:
                logger.log_operation_error(operation, component, e, **kwargs)
                raise
