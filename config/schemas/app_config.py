...
class SecurityConfig(BaseModel):
    ...
    jwt_secret_env: str = Field(
        default="JWT_SECRET_KEY", description="JWT密钥环境变量名"
    )
    developer_secret_env: str = Field(
        default="DEVELOPER_SECRET_KEY", description="开发者模式密钥环境变量名"
    )
    session_secret_env: str = Field(
        default="SESSION_SECRET_KEY", description="会话密钥环境变量名"
    )
    ...
    if PYDANTIC_V2:
        ...
        def validate_secrets(self):
            ...
            self.developer_secret_key = self.developer_secret_key or _read_secret_env(
                [
                    self.developer_secret_env,
                    DEFAULT_SECRET_ENV_NAMES["developer_secret_key"],
                ]
            )
            self.session_secret_key = self.session_secret_key or _read_secret_env(
                [
                    self.session_secret_env,
                    DEFAULT_SECRET_ENV_NAMES["session_secret_key"],
                ]
            )
            return self
    else:
        ...
        def validate_secrets(cls, values):
            ...
            values["developer_secret_env"] = (
                values.get("developer_secret_env") or "DEVELOPER_SECRET_KEY"
            )
            values["session_secret_env"] = (
                values.get("session_secret_env") or "SESSION_SECRET_KEY"
            )
            ...
            values["developer_secret_key"] = values.get(
            ...
            values["session_secret_key"] = values.get(
                "session_secret_key"
            ) or _read_secret_env(
                [
                    values["session_secret_env"],
...
class DeveloperConfig(BaseModel):
    ...
    enabled_env: str = Field(
        default="DEVELOPER_MODE_ENABLED", description="开发者模式开关环境变量"
    )
    debug_mode: bool = Field(default=False, description="开发者调试开关")
    debug_env: str = Field(
        default="DEVELOPER_DEBUG", description="开发者调试开关环境变量"
    )
    console_enabled: bool = Field(default=False, description="开发者控制台开关")
    console_env: str = Field(
        default="DEVELOPER_CONSOLE_ENABLED", description="开发者控制台环境变量"
    )
    ...
    if PYDANTIC_V2:
        ...
        def apply_env_overrides(cls, values):
            ...
            values["console_enabled"] = _env_flag(
                console_env, values.get("console_enabled", False)
            )
            return values
...
class PerformanceConfig(BaseModel):
    ...
    cache_type: Literal["memory", "redis", "multi"] = Field(
        default="memory", description="缓存类型"
    )
...
class AppConfig(BaseModel):
    ...
    if PYDANTIC_V2:
        ...
        @classmethod
        def apply_environment_default(cls, data):
            """Only use ENVIRONMENT when no explicit environment was supplied."""
            if isinstance(data, dict) and not data.get("environment"):
                return {**data, "environment": os.getenv("ENVIRONMENT", "development")}
            return data
    else:

        @root_validator(pre=True)
        def apply_environment_default(cls, values):
            """Only use ENVIRONMENT when no explicit environment was supplied."""
    ...
    if PYDANTIC_V2:
        ...
        def validate_environment_requirements(self):
            ...
            if env == "production":
                ...
                self.developer.enabled = _env_flag(
                    self.developer.enabled_env, self.developer.enabled
                )
                self.developer.debug_mode = _env_flag(
                    self.developer.debug_env, self.developer.debug_mode
                )
                self.developer.console_enabled = _env_flag(
                    self.developer.console_env, self.developer.console_enabled
                )
            ...
            if env == "production":
                ...
                if not self.security.jwt_secret_key:
                    missing_secrets.append(
                        self.security.jwt_secret_env
                        or DEFAULT_SECRET_ENV_NAMES["jwt_secret_key"]
                    )
                if not self.security.session_secret_key:
                    missing_secrets.append(
                        self.security.session_secret_env
                        or DEFAULT_SECRET_ENV_NAMES["session_secret_key"]
                    )
                if self.developer.enabled and not self.security.developer_secret_key:
                    missing_secrets.append(
                        self.security.developer_secret_env
        ...
        def validate_environment_requirements(cls, values):
            ...
            env = getattr(
                app_cfg, "environment", os.getenv("ENVIRONMENT", "development")
            )
            ...
            if env == "production":
                ...
                developer_cfg.enabled = _env_flag(
                    developer_cfg.enabled_env, developer_cfg.enabled
                )
                developer_cfg.debug_mode = _env_flag(
                    developer_cfg.debug_env, developer_cfg.debug_mode
                )
                developer_cfg.console_enabled = _env_flag(
                    developer_cfg.console_env, developer_cfg.console_enabled
                )
            ...
            if env == "production":
                ...
                if not security_cfg.jwt_secret_key:
                    missing_secrets.append(
                        getattr(security_cfg, "jwt_secret_env", None)
                        or DEFAULT_SECRET_ENV_NAMES["jwt_secret_key"]
                    )
                if not security_cfg.session_secret_key:
                    missing_secrets.append(
                        getattr(security_cfg, "session_secret_env", None)
                        or DEFAULT_SECRET_ENV_NAMES["session_secret_key"]
                    )
